        ...
        <div className="mx-auto flex min-h-[80vh] max-w-md items-center justify-center">
          <form onSubmit={login} className="w-full rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-2xl">
            <div className="mb-8 text-center">
              <Heart className="mx-auto mb-4 h-12 w-12 fill-red-500 text-red-500" />
              <h1 className="font-playfair text-4xl">Birthday Studio</h1>
              <p className="mt-2 text-white/50">لوحة إنشاء بطاقات عيد الميلاد</p>
            </div>
            ...
            <button className="flex w-full items-center justify-center gap-2 rounded-2xl bg-red-500 px-5 py-3 font-semibold transition hover:bg-red-400">
              <LockKeyhole size={18} /> دخول
            </button>
          </form>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-[#060010] px-5 py-10 text-white">
      <div className="mx-auto max-w-3xl">
        <div className="mb-8">
          <p className="text-sm uppercase tracking-[0.3em] text-red-300/70">Birthday Studio</p>
          <h1 className="mt-2 font-playfair text-4xl sm:text-5xl">إنشاء عيد ميلاد جديد</h1>
          <p className="mt-3 text-white/50">اسم + رسالة + صور + أغنية، والباقي تلقائي.</p>
        </div>
        ...
        <form onSubmit={create} className="space-y-6 rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-2xl sm:p-8">
          <div>
            <label className="mb-2 block text-sm text-white/70">اسم المستلم</label>
            <input value={name} onChange={e => setName(e.target.value)} required placeholder="مثلاً Sara" className="w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 outline-none focus:border-red-500/50" />
          </div>
          <div>
            <label className="mb-2 block text-sm text-white/70">الرسالة</label>
            <textarea value={message} onChange={e => setMessage(e.target.value)} required rows={6} placeholder="اكتب رسالتك هنا..." className="w-full resize-y rounded-2xl border border-white/10 bg-black/20 px-4 py-3 outline-none focus:border-red-500/50" />
          </div>
          <div>
            <label className="mb-2 flex items-center gap-2 text-sm text-white/70"><ImagePlus size={17} /> الصور</label>
            <input type="file" accept="image/*" multiple required onChange={e => setPhotos(Array.from(e.target.files || []))} className="block w-full cursor-pointer rounded-2xl border border-dashed border-white/15 bg-black/20 p-4 text-sm text-white/60" />
            {previews.length > 0 && <div className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-5">{previews.map(({ file, url }) => <div key={file.name + file.lastModified} className="aspect-square overflow-hidden rounded-xl border border-white/10"><img src={url} alt="preview" className="h-full w-full object-cover" /></div>)}</div>}
          </div>
          <div>
            <label className="mb-2 flex items-center gap-2 text-sm text-white/70"><Music2 size={17} /> الأغنية</label>
            <input type="file" accept="audio/*" required onChange={e => setMusic(e.target.files?.[0] || null)} className="block w-full cursor-pointer rounded-2xl border border-dashed border-white/15 bg-black/20 p-4 text-sm text-white/60" />
            {music && <p className="mt-2 text-sm text-white/50">{music.name}</p>}
          </div>
          {error && <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-200">{error}</div>}
          <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-red-500 px-5 py-4 font-semibold transition hover:bg-red-400 disabled:cursor-not-allowed disabled:opacity-50">
            <UploadCloud size={19} /> {loading ? 'جاري إنشاء البطاقة...' : 'إنشاء البطاقة'}
          </button>
        </form>
